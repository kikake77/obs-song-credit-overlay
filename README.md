...
> [!IMPORTANT]
> 出力先を選んだだけでは、文字はまだ入りません。目アイコンをONにしても、空のテキストへ文字はコピーされません。出力先を変更した後は、必ず下にある **OBSへ表示** を押してください。
...
**OBSへ表示** を押すと、選択した3個のテキストソースの目アイコンも自動でONになります。誤ってソースを非表示にしていても、曲名を含めて表示へ戻ります。
